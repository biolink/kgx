...
COUNT_BY_EDGE_PREDICATE = 'count_by_edge_predicate'
...
def summarize_edges(graph: nx.MultiDiGraph, facet_properties: Optional[List] = None):
    ...
    stats: Dict = {
        TOTAL_EDGES: 0,
        EDGE_PREDICATES: set(),
        COUNT_BY_EDGE_PREDICATE: {'unknown': {'count': 0}},
        COUNT_BY_SPO: {}
    }
    ...
    for u, v, k, data in graph.edges(keys=True, data=True):
        if 'predicate' not in data:
            stats[COUNT_BY_EDGE_PREDICATE]['unknown']['count'] += 1
            predicate = "unknown"
        else:
            predicate = data['predicate']
            stats[EDGE_PREDICATES].add(predicate)
            if predicate in stats[COUNT_BY_EDGE_PREDICATE]:
                stats[COUNT_BY_EDGE_PREDICATE][predicate]['count'] += 1
            else:
                stats[COUNT_BY_EDGE_PREDICATE][predicate] = {'count': 1}

            if facet_properties:
                for facet_property in facet_properties:
                    stats = get_facet_counts(data, stats, COUNT_BY_EDGE_PREDICATE, predicate, facet_property)
        ...
        key = f"{u_category}-{predicate}-{v_category}"
