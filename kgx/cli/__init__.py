...
from kgx.cli.cli_utils import get_input_file_types, parse_source, apply_operations, graph_summary, validate, \
...
def graph_summary_wrapper(inputs: List[str], input_format: str, input_compression: str, output: str, report_type: str, stream: bool, node_facet_properties: Optional[Set], edge_facet_properties: Optional[Set]):
    """
    ...
    """
    graph_summary(inputs, input_format, input_compression, output, report_type, stream, node_facet_properties=list(node_facet_properties), edge_facet_properties=list(edge_facet_properties))
...
@click.option('--input-format', required=False, help=f'The input format. Can be one of {get_input_file_types()}')
@click.option('--input-compression', required=False, help='The input compression type')
