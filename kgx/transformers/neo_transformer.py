...
class NeoTransformer(Transformer):
    ...
    def save_edge(self, edges_by_predicate: Dict[str, list], batch_size: int = 10000) -> None:
        """
        ...
        for predicate in edges_by_predicate.keys():
            query = self.generate_unwind_edge_query(predicate)
            log.info(query)
            edges = edges_by_predicate[predicate]
            time_start = current_time_in_millis()
    ...
    @staticmethod
    def generate_unwind_edge_query(predicate: str) -> str:
        """
    ...
    def save(self) -> None:
        ...
        edges_by_predicate: Dict[str, List] = {}
        for u, v, k, data in self.graph.edges(keys=True, data=True):
            self.validate_edge(data)
            predicate = data['predicate']
            if predicate in edges_by_predicate:
                edges_by_predicate[predicate].append(data)
            else:
                edges_by_predicate[predicate] = [data]
        ...
        # save all edges
        self.save_edge(edges_by_predicate)
