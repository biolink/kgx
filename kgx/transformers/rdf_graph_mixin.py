...
class RdfGraphMixin(object):
    ...
    def add_edge(self, subject_iri: URIRef, object_iri: URIRef, predicate_iri: URIRef, data: Optional[Dict[Any, Any]] = None) -> Dict:
        ...
        if ' ' in edge_predicate:
            log.debug(f"predicate IRI '{predicate_iri}' yields predicate '{edge_predicate}' that not in snake_case form; replacing ' ' with '_'")
        ...
        if edge_predicate_prefix not in {'biolink', 'rdf', 'rdfs', 'skos', 'owl'}:
            if PrefixManager.is_curie(edge_predicate):
                ...
                # if name:
                #     log.debug(f"predicate IRI '{predicate_iri}' yields predicate '{edge_predicate}' that is actually a CURIE; Using its mapping instead: {name}")
                #     predicate = f"{predicate_prefix}:{name}"
                # else:
                #     log.debug(f"predicate IRI '{predicate_iri}' yields predicate '{edge_predicate}' that is actually a CURIE; defaulting back to {self.DEFAULT_EDGE_PREDICATE}")
